...
from GLFlow.PWTF import build_model

from loss import criterion
from evaluate_GL import (validate_chairs, validate_sintel, validate_kitti,
...
def sequence_loss(output, flow_gt, valid, gamma=0.8, max_flow=400):
    ...
    #flow_loss += (valid[:, None] * (output['init'] - flow_gt).abs()).mean()
    ...
    epe = torch.sum((output['final'] - flow_gt) ** 2, dim=1).sqrt()
...
def get_args_parser():
    ...
    # Dataset
    parser.add_argument('--image_size', default=[352, 480], type=int, nargs='+')
    ...
    parser.add_argument('--train_iters', default=5, type=int)
    parser.add_argument('--val_iters', default=5, type=int)
...
def main(args):
    ...
    while total_steps < args.num_steps:
        ...
        for i, sample in enumerate(train_loader):
            ...
            output = model(img1, img2, iters=4, flow_gt=flow_gt, test_mode=False)

            loss, metrics = sequence_loss(output, flow_gt, valid, args.gamma)
            ...
            if args.local_rank == 0:
                ...
                logger.add_image_summary(img1, img2, output['flow'], flow_gt)
