...
class Model(nn.Module):
    def __init__(self,
        ...
        self.fmap_conv = nn.Conv2d(self.pretrain_dim // 2 * 5 + 3, 64, kernel_size=1, stride=1, padding=0, bias=True)
        ...
        self.warp_linear = nn.Conv2d(3 * 64 + 2 * 3 +2, 64, 1, 1, 0, bias=True)

        self.refine_net = VisionTransformer("vitt", 64, patch_size=8)

        self.refine_transform = nn.Conv2d(64+16, 64, 1, 1, 0, bias=True)
    ...
    def forward(self, image1, image2, iters=5, flow_gt=None, test_mode=False,):
        ...
        # run the feature network free feature; activae feature; depth
        fmap1_pretrain, fmap1_retrain, depth1 = self.encoder(image1)
        fmap2_pretrain, fmap2_retrain, depth2 = self.encoder(image2)
        ...
        fmap1_2x = self.fmap_conv(torch.cat([fmap1_pretrain, fmap1_retrain, fmap1_img, normal_map1], dim=1))  # torch.Size([1, 64, 168, 336])
        fmap2_2x = self.fmap_conv(torch.cat([fmap2_pretrain, fmap2_retrain, fmap2_img, normal_map2], dim=1))

        # 增强表面法向量在预测中的作用
        net = self.hidden_conv(torch.cat([fmap1_2x, normal_map1, fmap2_2x, normal_map2], dim=1))  # 128 + 6 - 64  torch.Size([1, 64, 168, 336])
        ...
        for itr in range(iters):
            ...
            net = self.refine_transform(torch.cat([refine_outs['out'], net], dim=1))  # torch.Size([1, 64, 168, 336])
            # 线性处理
...
time1 = time.time()
y = model(x1, x1, 4, gt, test_mode=True)
